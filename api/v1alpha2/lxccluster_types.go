...
type LXCClusterLoadBalancer struct {
	...
	//
	// When using keepalived, the controller will automatically inject /etc/keepalived/keepalived.conf into all control plane nodes of the cluster. This requires keepalived to be pre-installed on the image (it is included in the default images starting from version "v1.34.0").
	//
